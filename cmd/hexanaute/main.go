...
import (
	...
	"github.com/51TH-FireFox13/hexanaute/internal/css"
	...
	"github.com/51TH-FireFox13/hexanaute/internal/jsengine"
...
func main() {
	...
	fox.app = app.NewWithID("fr.hexanaute")
	fox.app.Settings().SetTheme(&foxTheme{})

	fox.window = fox.app.NewWindow(fmt.Sprintf("HexaNaute v%s", version))
...
func (f *FoxGUI) buildUI() {
	...
	f.statusLabel = widget.NewLabel("[HexaChain] —")
	...
	f.window.SetMainMenu(fyne.NewMainMenu(
		fyne.NewMenu("HexaNaute",
			fyne.NewMenuItem("Vérifier les mises à jour", f.checkUpdate),
			fyne.NewMenuItemSeparator(),
			fyne.NewMenuItem("Intégrité HexaChain", f.verifyChain),
			fyne.NewMenuItem("Infos HexaChain", f.showChainInfo),
			fyne.NewMenuItemSeparator(),
...
func (f *FoxGUI) onTabChanged(item *container.TabItem) {
	...
	if title == "" || title == "Nouvel onglet" {
		f.window.SetTitle(fmt.Sprintf("HexaNaute v%s", version))
	} else {
		f.window.SetTitle(fmt.Sprintf("HexaNaute — %s", title))
	}
}
...
func (f *FoxGUI) initHexaChain() {
	dir := f.foxchainDir()

	if foxchain.Exists(dir) {
		passEntry := widget.NewPasswordEntry()
		passEntry.SetPlaceHolder("Votre passphrase HexaChain")

		d := dialog.NewForm("HexaChain — Déverrouiller", "Ouvrir", "Sans profil",
			[]*widget.FormItem{
				widget.NewFormItem("Passphrase", passEntry),
			},
			func(ok bool) {
				if !ok {
					f.statusLabel.SetText("[HexaChain] Navigation sans profil")
					return
				}
				...
				if err != nil {
					...
					f.statusLabel.SetText("[HexaChain] Erreur")
					return
				}
				f.profile = p
				f.refreshBookmarks()
				...
				f.statusLabel.SetText(fmt.Sprintf("[HexaChain] %d blocs — OK", stats["total"]))
...
func (f *FoxGUI) showCreateProfile() {
	...
	d := dialog.NewForm("Créer un profil HexaChain", "Créer", "Annuler",
		[]*widget.FormItem{
		...
		func(ok bool) {
			...
			f.profile = p
			f.statusLabel.SetText("[HexaChain] Profil créé")
			dialog.ShowInformation("HexaChain",
...
func (f *FoxGUI) addCurrentBookmark() {
	...
	if f.profile == nil {
		dialog.ShowInformation("HexaChain", "Pas de profil actif.\nCréez-en un via le menu HexaNaute.", f.window)
		return
	}
...
func (f *FoxGUI) showAddPasswordDialog() {
	if f.profile == nil {
		dialog.ShowInformation("HexaChain", "Pas de profil actif.", f.window)
		return
	}
...
func (f *FoxGUI) checkUpdate() {
	f.guardLabel.SetText("[HexaOTA] Vérification...")
	...
	go func() {
		...
		if result.Error != nil {
			if result.Error == foxota.ErrNoUpdate {
				f.guardLabel.SetText("[HexaOTA] À jour")
				dialog.ShowInformation("HexaOTA",
					fmt.Sprintf("HexaNaute v%s est à jour.", version),
					f.window)
			} else {
		...
		if result.Available {
			f.guardLabel.SetText(fmt.Sprintf("[HexaOTA] v%s disponible !", result.Manifest.VersionStr))
...
func (f *FoxGUI) downloadUpdate(checker *foxota.Checker, checkResult *foxota.CheckResult) {
	f.guardLabel.SetText("[HexaOTA] Téléchargement...")
	...
	go func() {
		...
		if vr != nil && !vr.HashOK {
			...
			f.guardLabel.SetText("[HexaOTA] HASH INVALIDE")
			return
		}

		f.guardLabel.SetText("[HexaOTA] Application...")
		...
		f.guardLabel.SetText("[HexaOTA] Mise à jour appliquée !")
		dialog.ShowInformation("HexaOTA",
			fmt.Sprintf("v%s installée !\nRedémarrez HexaNaute.", checkResult.Manifest.VersionStr),
			f.window)
	}()
}
...
func (f *FoxGUI) verifyChain() {
	...
	if chain.Verify() {
		dialog.ShowInformation("Intégrité HexaChain",
...
func (f *FoxGUI) showChainInfo() {
	...
	dialog.ShowInformation("Infos HexaChain", info, f.window)
}

func (f *FoxGUI) showAbout() {
	dialog.ShowInformation("À propos",
		fmt.Sprintf(
			"HexaNaute v%s (Renard)\n\n"+
				"Navigateur souverain\n"+
				"Sécurisé par IA locale (AI Guard)\n"+
				"Stockage chiffré (HexaChain)\n"+
				"Mises à jour P2P signées (HexaOTA)\n\n"+
				"Licence EUPL v1.2\n"+
				"github.com/51TH-FireFox13/hexanaute",
			version),
		f.window)
}
...
func (f *FoxGUI) navigateInTab(tab *FoxTab, input string, addHistory bool) {
	...
	go func() {
		...
		if f.profile != nil {
			stats := f.profile.Vault().Stats()
			statusText = fmt.Sprintf("[HexaChain] %d blocs | %s", stats["total"], statusText)
		}
		if f.currentFoxTab() == tab {
		...
		// Sauvegarder dans l'historique HexaChain
		if f.profile != nil {
...
func (f *FoxGUI) buildNewTabContent() fyne.CanvasObject {
	// Logo
	logo := canvas.NewText("HexaNaute", color.NRGBA{R: 255, G: 140, B: 0, A: 255})
	logo.TextSize = 36
	...
	badges := container.NewHBox(
		...
		buildBadge("HexaChain", color.NRGBA{R: 80, G: 80, B: 200, A: 220}),
		buildBadge("HexaOTA P2P", color.NRGBA{R: 180, G: 80, B: 20, A: 220}),
...
func (f *FoxGUI) onClose() {
	...
	// Sauvegarder l'état des onglets dans HexaChain
	if f.profile != nil {
		var urls []string
		for _, tab := range f.foxTabs {
			if tab.pageURL != "" && !strings.HasPrefix(tab.pageURL, "about:") {
